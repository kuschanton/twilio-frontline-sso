...
export const TabAgents: React.FC<TabAgentsProps> = ({showToast}) => {
  ...
  const deleteWorker = async (phoneNumber: string) => {
    ...
    showToast('Successfully deleted worker')
  ...
  return (
    <StrictMode>
      <Theme.Provider theme='default'>
        ...
        <NewWorker isOpen={isOpen} handleClose={handleClose} refreshTable={handleRefreshTable} showToast={showToast}/>
      </Theme.Provider>
    </StrictMode>
  )
}
